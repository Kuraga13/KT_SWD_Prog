# ==============================================================================
#  STM32F4 flash read/write test script (PowerShell)
...
$TempDir = Join-Path ([System.IO.Path]::GetTempPath()) "f4_test_$(Get-Random)"
...
try {
    Write-Host "=== STM32F4 Flash Read/Write Test ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    # ------------------------------------------------------------------
    #  Generate test pattern (16 KB — one sector on all F4 variants)
    # ------------------------------------------------------------------
    $TestSize = 16384
    ...
    # --- Slow path (word-by-word SWD writes, no stub) ---
    ...
    #
    #  FLASH_OPTCR (4 bytes at 0x40023C14):
    #    Bit 0:     OPTLOCK (set by hardware, stripped by driver on write)
    #    Bit 1:     OPTSTRT (stripped by driver)
    #    Bits 3:2:  BOR_LEV  -- we toggle bit 2 as test marker
    #    Bit 5:     WDG_SW
    #    Bit 6:     nRST_STOP
    #    Bit 7:     nRST_STDBY
    #    Bits 15:8: RDP (must stay 0xAA = Level 0!)
    #    Bits 27:16: nWRP (0xFFF = all unprotected)
    # ------------------------------------------------------------------
    ...
    Invoke-CLI "read", "ob", "0x40023C14", "0x04", "-o", $OrigObBin
    $OrigOb = [System.IO.File]::ReadAllBytes($OrigObBin)
    Write-Host "  Current OPTCR: 0x$([BitConverter]::ToString($OrigOb[3..0]) -replace '-','')" -ForegroundColor Cyan
    ...
    Write-Host "  Test   OPTCR: 0x$([BitConverter]::ToString($TestOb[3..0]) -replace '-','')" -ForegroundColor Cyan

    # Write test value and read back
    Invoke-CLI "write", "ob", $TestObBin, "0x40023C14"
    Invoke-CLI "read", "ob", "0x40023C14", "0x04", "-o", $ReadObBin
    $ReadOb = [System.IO.File]::ReadAllBytes($ReadObBin)
    Write-Host "  Read   OPTCR: 0x$([BitConverter]::ToString($ReadOb[3..0]) -replace '-','')" -ForegroundColor Cyan

    # Compare (mask out OPTLOCK bit 0 — hardware always sets it on readback)
    $TestMatch = $true
    ...
    Invoke-CLI "write", "ob", $OrigObBin, "0x40023C14"
    Write-Host ""
