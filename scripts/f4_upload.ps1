# ==============================================================================
#  STM32F4 upload (restore) script (PowerShell)
#  Writes back: flash, option bytes from a previous dump
#  Source: dumps\f4\ directory
# ==============================================================================
...
if (-not $SrcDir) { $SrcDir = Join-Path $ScriptDir "..\dumps\f4" }
...
if (-not (Test-Path $FlashBin)) {
    Write-Error "Error: $FlashBin not found. Run f4_dump.ps1 first."
    exit 1
}
...
Write-Host "=== STM32F4 Upload (Restore) ===" -ForegroundColor Cyan
...
if (Test-Path $ObBin) {
    Invoke-CLI "write", "ob", $ObBin, "0x40023C14"
} else {
