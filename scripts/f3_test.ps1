# ==============================================================================
#  STM32F3 flash read/write test script (PowerShell)
...
$TempDir = Join-Path ([System.IO.Path]::GetTempPath()) "f3_test_$(Get-Random)"
...
try {
    Write-Host "=== STM32F3 Flash Read/Write Test ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    $RdpOutput = & $CLI "rdp" @ProbeOpts 2>&1 | Tee-Object -Variable RdpLines
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Command failed: kt_swd_cli rdp"
        exit 1
    }
    Write-Host ""

    # If RDP Level 1 is active, revert to Level 0 first (writes RDP=0xAA
    # option bytes, which triggers hardware mass erase + RDP regression)
    $RdpText = ($RdpLines | Out-String)
    if ($RdpText -match "Level 1") {
        Write-Host "  RDP Level 1 detected - reverting to Level 0..." -ForegroundColor DarkYellow
        $RdpBin = Join-Path $TempDir "rdp_level0.bin"
        # RDP=0xAA (Level 0), nRDP=0x55, rest = factory defaults
        [System.IO.File]::WriteAllBytes($RdpBin, [byte[]]@(
            0xAA, 0x55,   # RDP=Level0, nRDP
            0xFF, 0x00,   # USER defaults, nUSER
            0xFF, 0x00,   # Data0, nData0
            0xFF, 0x00,   # Data1, nData1
            0xFF, 0x00,   # WRP0, nWRP0
            0xFF, 0x00,   # WRP1, nWRP1
            0xFF, 0x00,   # WRP2, nWRP2
            0xFF, 0x00    # WRP3, nWRP3
        ))
        Invoke-CLI "write", "ob", $RdpBin, "0x1FFFF800"
        Write-Host "  RDP reverted - chip will reset and mass erase" -ForegroundColor Green
        # After regression, hardware erases option bytes and writes RDP=0xAA,
        # but WRP complement bytes are left invalid (0xFF/0xFF instead of 0xFF/0x00).
        # Write proper option bytes again to fix WRP complement pairs.
        Write-Host "  Writing valid option bytes after regression..." -ForegroundColor DarkYellow
        Invoke-CLI "write", "ob", $RdpBin, "0x1FFFF800"
        Remove-Item $RdpBin
        Write-Host "  Option bytes restored" -ForegroundColor Green
        Write-Host ""
    }
    ...
    #    [0] RDP    [1] nRDP    = 0xAA, 0x55  (Level 0 - no protection!)
