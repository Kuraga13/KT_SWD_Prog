# ==============================================================================
#  STM32F1 upload (restore) script (PowerShell)
#  Writes back: flash, option bytes from a previous dump
#  Source: dumps\f1\ directory
# ==============================================================================
...
if (-not $SrcDir) { $SrcDir = Join-Path $ScriptDir "..\dumps\f1" }
...
if (-not (Test-Path $FlashBin)) {
    Write-Error "Error: $FlashBin not found. Run f1_dump.ps1 first."
    exit 1
}
...
Write-Host "=== STM32F1 Upload (Restore) ===" -ForegroundColor Cyan
