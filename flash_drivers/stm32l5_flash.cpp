/**
 ******************************************************************************
 * @file           : stm32l5_flash.cpp
 * @brief          : Flash driver implementation for STM32L5 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
using namespace stm32l5_flash;

ProgrammerStatus Stm32L5FlashDriver::unlock(Transport& transport) {
...
ProgrammerStatus Stm32L5FlashDriver::lock(Transport& transport) {
    uint32_t cr;
...
ProgrammerStatus Stm32L5FlashDriver::unlockOptionBytes(Transport& transport) {
    auto status = flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY1);
    if (status != ProgrammerStatus::Ok) return status;
    return flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY2);
}

ProgrammerStatus Stm32L5FlashDriver::waitReady(Transport& transport) {
    return flash_wait_busy(transport, FLASH_SR, SR_BSY, SR_ALL_ERRORS, 100000, &m_error_);
}

ProgrammerStatus Stm32L5FlashDriver::erasePage(Transport& transport, uint32_t page_num, bool bank2) {
...
ProgrammerStatus Stm32L5FlashDriver::programDoubleWord(Transport& transport, uint32_t address,
...
RdpLevel Stm32L5FlashDriver::readRdpLevel(Transport& transport) {
    uint32_t optr;
    ...
    if (rdp == RDP_LEVEL_0_5) return RdpLevel::Level0;  // L5 level 0.5 treated as readable
    if (rdp == RDP_LEVEL_2) return RdpLevel::Level2;
    return RdpLevel::Level1;
}

ProgrammerStatus Stm32L5FlashDriver::eraseFlash(Transport& transport) {
...
ProgrammerStatus Stm32L5FlashDriver::writeFlash(Transport& transport, const uint8_t* data,
...
ProgrammerStatus Stm32L5FlashDriver::writeOptionBytes(Transport& transport, const uint8_t* data,
...
ProgrammerStatus Stm32L5FlashDriver::writeOtp(Transport& transport, const uint8_t* data,
                                                uint32_t address, uint32_t size) {
