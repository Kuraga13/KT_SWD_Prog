/**
 ******************************************************************************
 * @file           : stm32l4_flash.cpp
 * @brief          : Flash driver implementation for STM32L4 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
using namespace stm32l4_flash;

ProgrammerStatus Stm32L4FlashDriver::unlock(Transport& transport) {
...
ProgrammerStatus Stm32L4FlashDriver::lock(Transport& transport) {
    uint32_t cr;
...
ProgrammerStatus Stm32L4FlashDriver::unlockOptionBytes(Transport& transport) {
    auto status = flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY1);
    if (status != ProgrammerStatus::Ok) return status;
    return flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY2);
}

ProgrammerStatus Stm32L4FlashDriver::waitReady(Transport& transport) {
    return flash_wait_busy(transport, FLASH_SR, SR_BSY, SR_ALL_ERRORS, 100000, &m_error_);
}

ProgrammerStatus Stm32L4FlashDriver::erasePage(Transport& transport, uint32_t page_num, bool bank2) {
...
ProgrammerStatus Stm32L4FlashDriver::programDoubleWord(Transport& transport,
                                                        uint32_t address,
                                                        uint32_t word_lo,
                                                        uint32_t word_hi) {
...
RdpLevel Stm32L4FlashDriver::readRdpLevel(Transport& transport) {
    uint32_t optr;
...
ProgrammerStatus Stm32L4FlashDriver::eraseFlash(Transport& transport) {
...
ProgrammerStatus Stm32L4FlashDriver::writeFlash(Transport& transport,
                                                  const uint8_t* data,
                                                  uint32_t address,
                                                  uint32_t size) {
    auto status = unlock(transport);
...
ProgrammerStatus Stm32L4FlashDriver::writeOptionBytes(Transport& transport,
                                                        const uint8_t* data,
                                                        uint32_t address,
...
ProgrammerStatus Stm32L4FlashDriver::writeOtp(Transport& transport,
                                                const uint8_t* data,
                                                uint32_t address,
                                                uint32_t size) {
