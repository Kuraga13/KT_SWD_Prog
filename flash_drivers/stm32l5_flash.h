/**
 ******************************************************************************
 * @file           : stm32l5_flash.h
 * @brief          : Flash driver for STM32L5 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 ...
 * @details
 *
 * Flash driver for STM32L5 series (Cortex-M33, TrustZone).
 * Page-based flash with double-word (64-bit) programming.
 * Similar to L4 flash controller with added TrustZone secure/non-secure
 * register banking.
 *
 * Features:
 * - Page erase (2 KB single / 4 KB dual bank) and mass erase
 * - Double-word (64-bit) flash programming
 * - Dual-bank support
 * - Option byte read/write with RDP, WRP, PCROP, secure area support
 * - TrustZone-aware secure/non-secure flash register access
 *
 * Usage Example:
 * ```cpp
 * Stm32L5FlashDriver flash_driver;
...
namespace stm32l5_flash {
    constexpr uint32_t FLASH_BASE       = 0x40022000;
    constexpr uint32_t FLASH_BASE_NS    = 0x40022000;   // non-secure alias
    constexpr uint32_t FLASH_BASE_S     = 0x50022000;   // secure alias
    ...
    constexpr uint32_t FLASH_WRP1AR     = FLASH_BASE + 0x28;
    constexpr uint32_t FLASH_WRP1BR     = FLASH_BASE + 0x2C;
    constexpr uint32_t FLASH_WRP2AR     = FLASH_BASE + 0x48;
    constexpr uint32_t FLASH_WRP2BR     = FLASH_BASE + 0x4C;
    constexpr uint32_t FLASH_SECWM1R1   = FLASH_BASE + 0x70;
    constexpr uint32_t FLASH_SECWM1R2   = FLASH_BASE + 0x74;
    constexpr uint32_t FLASH_SECWM2R1   = FLASH_BASE + 0x78;
    constexpr uint32_t FLASH_SECWM2R2   = FLASH_BASE + 0x7C;
    ...
    constexpr uint32_t CR_BKER          = (1UL << 11);
    ...
    constexpr uint32_t PAGE_SIZE        = 2048;          // 2 KB (single bank)
    constexpr uint32_t PAGE_SIZE_DUAL   = 4096;          // 4 KB (dual bank)
    constexpr uint32_t OB_BASE          = 0x40022040;
    constexpr uint32_t OB_SIZE          = 128;
    constexpr uint32_t OTP_BASE         = 0x0BFA0000;
    constexpr uint32_t OTP_SIZE         = 512;

    constexpr uint32_t PROG_WIDTH       = 8;             // double-word (64-bit)

    constexpr uint8_t  RDP_LEVEL_0      = 0xAA;
    constexpr uint8_t  RDP_LEVEL_0_5    = 0x55;          // L5 has RDP level 0.5
    constexpr uint8_t  RDP_LEVEL_2      = 0xCC;
}

class Stm32L5FlashDriver : public FlashDriver {
public:
