/**
 ******************************************************************************
 * @file           : stm32wb_flash.cpp
 * @brief          : Flash driver implementation for STM32WB series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
using namespace stm32wb_flash;

ProgrammerStatus Stm32WbFlashDriver::unlock(Transport& transport) {
...
ProgrammerStatus Stm32WbFlashDriver::lock(Transport& transport) {
    uint32_t cr;
...
ProgrammerStatus Stm32WbFlashDriver::unlockOptionBytes(Transport& transport) {
    auto status = flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY1);
    if (status != ProgrammerStatus::Ok) return status;
    return flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY2);
}

ProgrammerStatus Stm32WbFlashDriver::waitReady(Transport& transport) {
    return flash_wait_busy(transport, FLASH_SR, SR_BSY | SR_CFGBSY, SR_ALL_ERRORS, 100000, &m_error_);
}

ProgrammerStatus Stm32WbFlashDriver::erasePage(Transport& transport, uint32_t page_num) {
...
ProgrammerStatus Stm32WbFlashDriver::programDoubleWord(Transport& transport, uint32_t address,
...
RdpLevel Stm32WbFlashDriver::readRdpLevel(Transport& transport) {
    uint32_t optr;
...
ProgrammerStatus Stm32WbFlashDriver::eraseFlash(Transport& transport) {
...
ProgrammerStatus Stm32WbFlashDriver::writeFlash(Transport& transport, const uint8_t* data,
...
ProgrammerStatus Stm32WbFlashDriver::writeOptionBytes(Transport& transport, const uint8_t* data,
...
ProgrammerStatus Stm32WbFlashDriver::writeOtp(Transport& transport, const uint8_t* data,
                                                uint32_t address, uint32_t size) {
