 ...
 * @brief          : Flash driver for STM32WB series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 ...
 * @details
 *
 * Flash driver for STM32WB series (dual-core Cortex-M4 + M0+, BLE/802.15.4).
 * Page-based flash with double-word (64-bit) programming.
 * Flash is shared between CPU1 (M4) and CPU2 (M0+, wireless stack).
 *
 * Features:
 * - Page erase (4 KB pages) and mass erase
 * - Double-word (64-bit) flash programming
 * - Option byte read/write with RDP, WRP, PCROP support
 * - CPU2 secure flash area (wireless stack) awareness
 * - OTP area access
 *
 * Usage Example:
 * ```cpp
 * Stm32WbFlashDriver flash_driver;
...
namespace stm32wb_flash {
    ...
    constexpr uint32_t FLASH_SFR        = FLASH_BASE + 0x80;  // secure flash area
    ...
    constexpr uint32_t PAGE_SIZE        = 4096;          // 4 KB
...
class Stm32WbFlashDriver : public FlashDriver {
public:
