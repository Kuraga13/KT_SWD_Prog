 ...
 * @brief          : Flash driver for STM32H5 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 ...
 * @details
 *
 * Flash driver for STM32H5 series (Cortex-M33, high-performance).
 * Sector-based flash with quad-word (128-bit = 16 bytes) programming.
 *
 * Features:
 * - Sector erase (8 KB) and mass erase
 * - Quad-word (128-bit) flash programming
 * - Dual-bank architecture
 * - Option byte read/write with RDP, WRP, secure area support
 * - TrustZone-aware register access
 *
 * Usage Example:
 * ```cpp
 * Stm32H5FlashDriver flash_driver;
...
namespace stm32h5_flash {
    constexpr uint32_t FLASH_BASE       = 0x40022000;
    constexpr uint32_t FLASH_BASE_S     = 0x50022000;
    ...
    constexpr uint32_t CR_BER           = (1UL << 2);
    constexpr uint32_t CR_SNB_SHIFT     = 3;
    constexpr uint32_t CR_SNB_MASK      = 0x7F;
    constexpr uint32_t CR_BKER          = (1UL << 11);
    constexpr uint32_t CR_MER           = (1UL << 15);
    ...
    constexpr uint32_t PROG_WIDTH       = 16;            // quad-word (128-bit)

    constexpr uint8_t  RDP_LEVEL_0      = 0xAA;
    constexpr uint8_t  RDP_LEVEL_0_5    = 0x55;
    constexpr uint8_t  RDP_LEVEL_2      = 0xCC;
}

class Stm32H5FlashDriver : public FlashDriver {
public:
...
private:
    ...
    ProgrammerStatus eraseSector(Transport& transport, uint32_t sector, bool bank2 = false);
    ProgrammerStatus programQuadWord(Transport& transport, uint32_t address,
