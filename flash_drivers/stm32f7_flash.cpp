/**
 ******************************************************************************
 * @file           : stm32f7_flash.cpp
 * @brief          : Flash driver implementation for STM32F7 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
using namespace stm32f7_flash;

ProgrammerStatus Stm32F7FlashDriver::unlock(Transport& transport) {
...
ProgrammerStatus Stm32F7FlashDriver::lock(Transport& transport) {
    uint32_t cr;
...
ProgrammerStatus Stm32F7FlashDriver::unlockOptionBytes(Transport& transport) {
    auto status = flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY1);
    if (status != ProgrammerStatus::Ok) return status;
    return flash_write_word(transport, FLASH_OPTKEYR, OPT_KEY2);
}

ProgrammerStatus Stm32F7FlashDriver::waitReady(Transport& transport) {
    return flash_wait_busy(transport, FLASH_SR, SR_BSY,
                           SR_PGSERR | SR_PGPERR | SR_PGAERR | SR_WRPERR | SR_OPERR,
                           100000, &m_error_);
}

ProgrammerStatus Stm32F7FlashDriver::eraseSector(Transport& transport, uint8_t sector) {
...
RdpLevel Stm32F7FlashDriver::readRdpLevel(Transport& transport) {
    uint32_t optcr;
...
ProgrammerStatus Stm32F7FlashDriver::eraseFlash(Transport& transport) {
...
ProgrammerStatus Stm32F7FlashDriver::writeFlash(Transport& transport,
                                                  const uint8_t* data,
                                                  uint32_t address,
                                                  uint32_t size) {
    auto status = unlock(transport);
...
ProgrammerStatus Stm32F7FlashDriver::writeOptionBytes(Transport& transport,
                                                        const uint8_t* data,
                                                        uint32_t address,
                                                        uint32_t size,
                                                        bool unsafe) {
...
ProgrammerStatus Stm32F7FlashDriver::writeOtp(Transport& transport,
                                                const uint8_t* data,
                                                uint32_t address,
                                                uint32_t size) {
