 ...
 * @brief          : Flash driver for STM32F3 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
namespace stm32f3_flash {
    ...
    constexpr uint32_t CR_OBL_LAUNCH    = (1UL << 13);
    ...
    constexpr uint32_t OTP_BASE         = 0x1FFFF800;
...
// SRAM flash loader stub layout (conservative: 12 KB SRAM, smallest supported F3)
namespace f3_flash_loader {
    ...
    constexpr uint32_t SRAM_MIN     = 12288;                   // 12 KB (smallest supported F3)
    constexpr uint32_t SRAM_TOP     = SRAM_BASE + SRAM_MIN;   // stack top
    constexpr uint32_t BUFFER_SIZE  = SRAM_TOP - BUFFER_ADDR;  // 12032 bytes
    ...
    // Cortex-M4 Thumb flash loader stub (80 bytes)
    // Programs flash half-words using PG mode, then halts via BKPT.
    // Same flash controller as F1 (identical register addresses and bits).
    ...
    constexpr uint8_t stub[] = {
        ...
        0x14, 0x27,             // MOVS   R7, #0x14        ; PGERR | WRPERR
        0x38, 0x42,             // TST    R0, R7
        0x08, 0xD1,             // BNE    error
