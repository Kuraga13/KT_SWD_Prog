/**
 ******************************************************************************
 * @file           : stm32l0_flash.cpp
 * @brief          : Flash driver implementation for STM32L0 series
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
...
using namespace stm32l0_flash;

ProgrammerStatus Stm32L0FlashDriver::unlockPE(Transport& transport) {
...
ProgrammerStatus Stm32L0FlashDriver::unlockProgram(Transport& transport) {
    // Must unlock PE first, then program memory
...
ProgrammerStatus Stm32L0FlashDriver::unlockOptionBytes(Transport& transport) {
    auto status = unlockPE(transport);
    if (status != ProgrammerStatus::Ok) return status;
...
ProgrammerStatus Stm32L0FlashDriver::lock(Transport& transport) {
    uint32_t pecr;
...
ProgrammerStatus Stm32L0FlashDriver::waitReady(Transport& transport) {
    return flash_wait_busy(transport, FLASH_SR, SR_BSY, SR_ALL_ERRORS,
                           100000, &m_error_);
}

ProgrammerStatus Stm32L0FlashDriver::erasePage(Transport& transport, uint32_t address) {
    // Set ERASE + PROG in PECR, then write to page start address
...
RdpLevel Stm32L0FlashDriver::readRdpLevel(Transport& transport) {
    uint32_t optr;
...
ProgrammerStatus Stm32L0FlashDriver::eraseFlash(Transport& transport) {
    auto status = unlockProgram(transport);
    if (status != ProgrammerStatus::Ok) return status;

    // L0 has no single mass-erase bit — erase page by page
    // Caller should determine flash size and call erasePage for each
    // For now, erase first 64 KB (512 pages of 128 bytes)
    for (uint32_t page = 0; page < 512; page++) {
        ...
        if (!reportProgress((page + 1) * PAGE_SIZE, 512 * PAGE_SIZE)) {
...
ProgrammerStatus Stm32L0FlashDriver::writeFlash(Transport& transport,
                                                  const uint8_t* data,
                                                  uint32_t address,
                                                  uint32_t size) {
    auto status = unlockProgram(transport);
...
ProgrammerStatus Stm32L0FlashDriver::writeOptionBytes(Transport& transport,
                                                        const uint8_t* data,
                                                        uint32_t address,
...
ProgrammerStatus Stm32L0FlashDriver::writeOtp(Transport& transport,
                                                const uint8_t* data,
                                                uint32_t address,
                                                uint32_t size) {
    m_error_ = "OTP not available on this chip family";
    return ProgrammerStatus::ErrorWrite;  // L0 has no dedicated OTP
}
